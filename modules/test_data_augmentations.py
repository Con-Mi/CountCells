...
for i in tqdm(range(len(rotations)), total=len(rotations)):
    for j, sample in tqdm(enumerate(data), total=len(data)):
        ...
        new_img.save("../data/AugmData/augm_flip" + str(i) + str(j) + "_.png")
        new_label.save("../data/AugmData/augm_flip_label_" + str(i) + str(j) + "_.png")
...
for i in tqdm(range(len(blur)), total=len(blur)):
    for j, sample in tqdm(enumerate(data), total=len(data)):
        ...
        new_img.save("../data/AugmData/augm_blur" + str(i) + str(j) + "_.png")
        label.save("../data/AugmData/augm_blur_label_"+ str(i) + str(j) + ".png")
...
for i in tqdm(range(len(misc)), total=len(misc)):
    for j, sample in tqdm(enumerate(data), total=len(data)):
        ...
        new_img = Image.fromarray(new_img)
        label = Image.fromarray(label)
        new_img.save("../data/AugmData/augm_sharp" + str(i) + str(j) + "_.png")
        label.save("../data/AugmData/augm_sharp_label_"+ str(i) + str(j) + ".png")
...
for j, sample in tqdm(enumerate(data), total=len(data)):
    ...
    new_img = Image.fromarray(new_img)
    label = Image.fromarray(label)
    new_img.save("../data/AugmData/augm_exp" + str(j) + "_.png")
    label.save("../data/AugmData/augm_exp_label_" + str(j) + ".png")
print("Finished..")
